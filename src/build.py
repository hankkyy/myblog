...
T = {
    "en": {
        ...
        "about_projects": "Projects Involved",
        ...
        "chat_welcome": "Hey! I'm Hank's digital avatar. How'd you find your way here?",
        "chat_send": "Send",
        ...
        "profile_sort_recent": "Recent",
        "profile_sort_sessions": "Most Visits",
        "profile_sort_messages": "Most Messages",
        "profile_sort_first": "First Seen",
        "profile_filter_all": "All Types",
        "profile_filter_reader": "Reader",
        "profile_filter_colleague": "Colleague",
        ...
        "profile_anonymous_name": "Anonymous Visitor",
        "profile_anonymous_desc": "No personal info shared",
        "profile_label_personality": "Personality",
        "profile_label_keyfacts": "Key Facts",
        "profile_label_interests": "Interests",
        "profile_label_activity": "Activity",
        "profile_label_evolution": "Evolution",
        "profile_session_unit": "session",
        "profile_sessions_unit": "sessions",
        "profile_messages_unit": "messages",
        "profile_no_analysis": "No analysis available",
        "profile_view_chat": "View Chat →",
        "profile_snapshots_unit": "snapshots",
        "insight_profiles": "👤 Profiles",
        "insight_sessions": "💬 Total Sessions",
        "insight_avg_visits": "🔄 Avg Visits",
        "insight_hot_topics": "🔥 Hot Topics",
        "log_visitor_prefix": "Visitor — ",
        "log_msgs_unit": "msgs",
    ...
    "zh": {
        ...
        "profile_sort_recent": "最近来访",
        "profile_sort_sessions": "回访次数",
        "profile_sort_messages": "消息总数",
        "profile_sort_first": "最早发现",
        "profile_filter_all": "全部类型",
        "profile_filter_reader": "读者",
        "profile_filter_colleague": "同行",
        "profile_filter_friend": "朋友",
        "profile_filter_classmate": "同学",
        "profile_filter_stranger": "陌生人",
        "profile_filter_anonymous": "匿名（未透露信息）",
        "profile_anonymous_name": "匿名访客",
        "profile_anonymous_desc": "未透露个人信息",
        "profile_label_personality": "性格特征",
        "profile_label_keyfacts": "关键事实",
        "profile_label_interests": "兴趣话题",
        "profile_label_activity": "活跃度",
        "profile_label_evolution": "画像演变",
        "profile_session_unit": "次会话",
        "profile_sessions_unit": "次会话",
        "profile_messages_unit": "条消息",
        "profile_no_analysis": "暂无分析",
        "profile_view_chat": "查看对话记录 →",
        "profile_snapshots_unit": "次快照",
        "insight_profiles": "👤 画像总数",
        "insight_sessions": "💬 总会话数",
        "insight_avg_visits": "🔄 平均回访",
        "insight_hot_topics": "🔥 热门话题",
        "log_visitor_prefix": "访客 — ",
        "log_msgs_unit": "条消息",
  ...
  <div id="chat-panel">
    <div id="chat-header">
      ...
      <div id="chat-header-actions">
        <button id="chat-clear-btn" onclick="clearConversation()" title="{t['chat_clear']}">
  ...
  try{{
    ...
    if(e.name==='AbortError'){{
      ...
      if(lastBubble&&lastBubble.classList.contains('streaming')){{
        ...
        if(!lastBubble.textContent.trim())lastBubble.innerHTML='<em>{t['chat_stopped']}</em>';
      }}else{{
        addMessage('assistant','{t['chat_stopped']}');
      }}
    }}else{{
      addErrorMessage();
    }}
...
function addErrorMessage(){{
  ...
  div.innerHTML='<div class="chat-bubble chat-error-bubble"><span>{t['chat_error_unavailable']}</span> <button class="chat-retry-btn" onclick="retryLastMessage()">{t['chat_retry']}</button></div>';
  messages.appendChild(div);
  requestAnimationFrame(function(){{messages.scrollTop=messages.scrollHeight;}});
}}
...
<style>
.log-entry{{border:none;border-radius:16px;margin-bottom:16px;overflow:hidden;background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.04),0 0 0 1px rgba(0,0,0,.02)}}
.log-entry-header{{padding:16px 20px;cursor:pointer;user-select:none;background:#fff;transition:all .15s}}
.log-entry-header:hover{{background:#f8f9fb}}
.log-entry-summary{{margin-bottom:6px}}
.log-preview{{font-size:13px;color:#1a1a2e;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
.log-entry-meta{{display:flex;align-items:center;gap:12px;font-size:12px}}
.log-sid{{font-family:monospace;color:#6b7280;background:#f1f3f5;padding:2px 8px;border-radius:6px;font-size:11px}}
.log-time{{color:#9ca3af}}
.log-msgs{{color:#6b7280;font-weight:500}}
.log-chevron{{transition:transform .2s;font-size:12px;color:#9ca3af;margin-left:auto}}
.log-entry.open .log-chevron{{transform:rotate(180deg)}}
.log-entry.open .log-entry-header{{background:#f8f9fb}}
.log-entry-body{{display:none;padding:24px 20px;background:#fbfbfb;border-top:1px solid #f0f0f3}}
...
.log-bubble{{max-width:82%;padding:12px 16px;border-radius:16px;font-size:13px;line-height:1.6}}
.log-msg.user .log-bubble{{background:#6366f1;color:#fff;border-bottom-right-radius:6px}}
.log-msg.assistant .log-bubble{{background:#fff;color:#363b40;border:1px solid #f0f0f3;border-bottom-left-radius:6px}}
...
/* Admin tabs */
.admin-tab{{background:none;border:none;color:#9ca3af;padding:8px 18px;font-size:13px;cursor:pointer;border-radius:8px;transition:all .15s;font-weight:500}}
.admin-tab:hover{{color:#6b7280;background:rgba(0,0,0,.03)}}
.admin-tab.active{{color:#6366f1;background:#eef2ff}}
/* Profile cards */
.profile-card{{border:none;border-radius:16px;margin-bottom:14px;overflow:hidden;background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.04),0 0 0 1px rgba(0,0,0,.02);transition:box-shadow .15s}}
.profile-card:hover{{box-shadow:0 4px 12px rgba(0,0,0,.06),0 0 0 1px rgba(0,0,0,.04)}}
.profile-card-header{{padding:18px 44px 18px 20px;cursor:pointer;user-select:none;background:#fff;transition:background .15s;position:relative}}
.profile-card-header:hover{{background:#f8f9fb}}
.profile-card-top{{display:flex;justify-content:space-between;align-items:center;margin-bottom:10px}}
.profile-name{{font-size:16px;font-weight:700;color:#1a1a2e;display:flex;align-items:center}}
.profile-returning-badge{{font-size:11px;font-weight:600;color:#6366f1;background:#eef2ff;padding:2px 8px;border-radius:10px;margin-left:8px}}
.profile-time{{font-size:12px;color:#9ca3af}}
.profile-tags{{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:4px}}
.profile-tag{{font-size:12px;padding:3px 10px;border-radius:8px;font-weight:500}}
.profile-tag-occ{{background:#eef2ff;color:#4f46e5}}
.profile-tag-loc{{background:#fef3c7;color:#d97706}}
.profile-tag-rel{{background:#f3e8ff;color:#7c3aed}}
.profile-tag-int{{background:#d1fae5;color:#059669}}
.profile-facts{{display:flex;flex-wrap:wrap;gap:6px}}
.profile-fact{{font-size:12px;background:#f3f4f6;color:#6b7280;padding:3px 10px;border-radius:6px}}
.profile-chevron{{transition:transform .2s;font-size:12px;color:#9ca3af;position:absolute;top:22px;right:20px}}
.profile-card.open .profile-chevron{{transform:rotate(180deg)}}
.profile-card.open .profile-card-header{{background:#f8f9fb}}
.profile-card-body{{display:none;padding:24px 20px;background:#fbfbfb;border-top:1px solid #f0f0f3}}
.profile-card.open .profile-card-body{{display:block}}
.profile-section{{margin-bottom:18px}}
.profile-section:last-child{{margin-bottom:0}}
.profile-section-label{{font-size:11px;text-transform:uppercase;letter-spacing:.6px;color:#9ca3af;margin-bottom:6px;font-weight:600}}
.profile-section-text{{font-size:13px;color:#363b40;line-height:1.65}}
.profile-interest-tag{{display:inline-block;font-size:12px;background:#d1fae5;color:#059669;padding:3px 10px;border-radius:8px;margin:3px 5px 3px 0}}
.profile-view-chat-btn{{background:#eef2ff;color:#4f46e5;border:none;padding:8px 16px;border-radius:8px;font-size:12px;cursor:pointer;margin-top:10px;font-weight:600;transition:all .15s}}
.profile-view-chat-btn:hover{{background:#e0e7ff;transform:translateY(-1px)}}
.profile-history-item{{padding:10px 0;border-bottom:1px solid #f0f0f3;font-size:12px}}
.profile-history-item:last-child{{border-bottom:none}}
.profile-history-time{{color:#9ca3af;font-size:11px;margin-bottom:6px}}
.profile-change-tag{{display:inline-block;background:#fef3c7;color:#d97706;padding:3px 8px;border-radius:6px;font-size:11px;margin:2px 5px 2px 0}}
#admin-profiles-toolbar{{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px}}
#admin-profiles-toolbar select{{padding:8px 12px;border-radius:10px;border:1.5px solid #e5e7eb;font-size:13px;background:#fff;color:#6b7280;cursor:pointer;transition:border-color .15s,box-shadow .15s}}
#admin-profiles-toolbar select:hover{{border-color:#6366f1}}
#admin-profiles-toolbar select:focus{{outline:none;border-color:#6366f1;box-shadow:0 0 0 3px rgba(99,102,241,.1)}}
#admin-insights{{display:none;padding:20px;background:#fff;border-radius:16px;margin-bottom:20px;box-shadow:0 1px 2px rgba(0,0,0,.04),0 0 0 1px rgba(0,0,0,.02)}}
.insight-stat{{background:#f8f9fb;border-radius:14px;padding:16px 20px;min-width:120px;text-align:center;position:relative;overflow:hidden;flex:1;transition:all .15s}}
.insight-stat:hover{{background:#eef2ff}}
.insight-stat-value{{font-size:28px;font-weight:800;color:#1a1a2e;line-height:1.1}}
.insight-stat-label{{font-size:11px;color:#9ca3af;margin-top:4px;font-weight:500}}
</style>
  ...
  <div id="admin-panel">
    <div id="admin-header">
      ...
      <div style="display:flex;gap:8px">
        <button id="admin-logout-btn" onclick="lockAdmin()" style="display:none;background:rgba(255,255,255,.12);color:#fff;border:1px solid rgba(255,255,255,.2);border-radius:8px;padding:5px 14px;font-size:12px;cursor:pointer;font-weight:500;transition:all .15s">{t['admin_logout']}</button>
        <button onclick="closeAdmin()" style="background:none;border:none;color:rgba(255,255,255,.7);cursor:pointer;font-size:20px;line-height:1;padding:4px;transition:color .15s" title="{t['admin_close']}">✕</button>
      </div>
    </div>
    <div id="admin-tabs" style="display:none;display:flex;gap:4px;border-bottom:1px solid #e5e7eb;padding:8px 28px">
      <button class="admin-tab" data-tab="logs" onclick="switchAdminTab('logs')">{t['admin_tab_logs']}</button>
      <button class="admin-tab active" data-tab="profiles" onclick="switchAdminTab('profiles')">{t['admin_tab_profiles']}</button>
    </div>
    <div id="admin-body">
      <div id="admin-login">
      ...
      <div id="admin-profiles" style="display:none">
        <div id="admin-profiles-toolbar">
          <select id="profile-sort" onchange="loadProfiles(1)">
            <option value="recent">{t['profile_sort_recent']}</option>
            <option value="sessions">{t['profile_sort_sessions']}</option>
            <option value="messages">{t['profile_sort_messages']}</option>
            <option value="first">{t['profile_sort_first']}</option>
          </select>
          <select id="profile-filter-rel" onchange="loadProfiles(1)">
            <option value="all">{t['profile_filter_all']}</option>
            <option value="reader">{t['profile_filter_reader']}</option>
            <option value="colleague">{t['profile_filter_colleague']}</option>
        ...
        <div id="admin-insights">
          <div style="display:flex;gap:16px;flex-wrap:wrap">
            <div class="insight-stat"><div class="insight-stat-value" id="insight-total">-</div><div class="insight-stat-label">{t['insight_profiles']}</div></div>
            <div class="insight-stat"><div class="insight-stat-value" id="insight-sessions">-</div><div class="insight-stat-label">{t['insight_sessions']}</div></div>
            <div class="insight-stat"><div class="insight-stat-value" id="insight-avg">-</div><div class="insight-stat-label">{t['insight_avg_visits']}</div></div>
          </div>
...
window.loadInsights=async function(){{
  try{{
    ...
    var interestsHtml='<strong>'+t['insight_hot_topics']+': </strong>';
    if(!ins.topInterests||ins.topInterests.length===0){{
      interestsHtml+='<span style="color:#adb5bd">-</span>';
    }}else{{
...
// Helper: safely extract a number from EJSON objects or corrupted strings
function safeNumber(val, fallback){{
  ...
  if(typeof val==='string'){{
    var m=val.match(/\\d+/);
    return m?Number(m[0]):(fallback||0);
  }}
  return fallback||0;
}}
...
function renderProfileCard(p){{
  ...
  var name=isAnon?t['profile_anonymous_name']:(p.profile&&p.profile.name?p.profile.name:'Anonymous');
  var occ=p.profile&&p.profile.occupation?p.profile.occupation:'';
  var loc=p.profile&&p.profile.location?p.profile.location:'';
  var pers=isAnon?t['profile_anonymous_desc']:(p.profile&&p.profile.personality?p.profile.personality:'');
  ...
  var sc=safeNumber(p.sessionCount,0);
  var returningBadge=sc>1?' <span class="profile-returning-badge">↻'+sc+'</span>':'';
  var totalMsgs=safeNumber(p.totalMessages||p.messageCount,0);
  var sessionLabel=sc===1?t['profile_session_unit']:t['profile_sessions_unit'];
  var sessionInfo='<div class="profile-section"><div class="profile-section-label">'+t['profile_label_activity']+'</div><div class="profile-section-text" style="font-family:monospace;font-size:12px;color:#6b7280">'+sc+' '+sessionLabel+' · '+totalMsgs+' '+t['profile_messages_unit']+'</div></div>';
  var historyHtml='';
  if(p.history&&p.history.length>1){{
    historyHtml='<div class="profile-section"><div class="profile-section-label">'+t['profile_label_evolution']+' ('+p.history.length+' '+t['profile_snapshots_unit']+')</div>';
...
function renderLogEntry(log){{
  ...
  var timeStr=date.toLocaleString();
  var title=t['log_visitor_prefix']+timeStr;
  entry.innerHTML='<div class="log-entry-header" onclick="this.parentElement.classList.toggle(&#39;open&#39;)"><div class="log-entry-summary"><span class="log-preview">'+title+'</span></div><div class="log-entry-meta"><span class="log-sid">#'+log.sessionId.slice(-8)+'</span><span class="log-time">'+timeStr+'</span><span class="log-msgs">'+(log.messages?log.messages.length:0)+' '+t['log_msgs_unit']+'</span><span class="log-chevron">▼</span></div></div>';
