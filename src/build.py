...
window.toggleChat=function(){{
  ...
  if(isOpen){{
    ...
    // Restore body scroll + reset any keyboard‑adjusted styles
...
// Mobile: keep chat input above keyboard (iOS Safari)
if(window.visualViewport){{
  ...
  window.visualViewport.addEventListener('resize',function(){{
    ...
    resizeTimer=setTimeout(function(){{
      ...
      if(!isMobile)return;
      // On mobile full-screen panel, lock bottom to visual viewport height
      var vh=window.visualViewport.height;
      var offsetTop=window.visualViewport.offsetTop;
      panel.style.height=vh+'px';
      panel.style.top=offsetTop+'px';
      // Scroll to bottom so latest messages + input are visible
      messages.scrollTop=messages.scrollHeight;
    }},100);
  }});
}}
