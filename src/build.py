...
window.showRefreshHint=showRefreshHint;
...
window.hideRefreshHint=hideRefreshHint;
...
window.sendMessage=async function(){{
  ...
  // ---- Client-side critical fact cache ----
  // Before calling the API, check if the user asked a known factual question.
  // If matched, answer instantly — saves an API call and eliminates fabrication risk.
  var localAnswer=checkLocalCriticalFact(text);
  if(localAnswer){{
    typing.remove();
    finishStream();
    var localDiv=addMessage('assistant','',true);
    var localBubble=localDiv.querySelector('.chat-bubble');
    localBubble.innerHTML=renderMarkdown(localAnswer);
    // Save the local answer to chat log (fire-and-forget)
    try{{
      var cacheHistory=[];
      var cacheMsgs=messages.querySelectorAll('.chat-msg:not(.typing):not(.chat-welcome-msg)');
      cacheMsgs.forEach(function(m){{
...
// ---- Critical fact local cache ----
// Answers known factual questions instantly without calling the API.
// Kept in sync with api/chat.js CRITICAL_ANSWER.
window.checkLocalCriticalFact=function(text){{
  // Nickname origin
  ...
  if(/做什么.{{0,6}}的|什么工作|做什么工作|什么职业|what.*do|what.*job|工作是|职业是/i.test(text))
    return '他是数据平台工程师，主要做数据基础设施和 AI Agent 开发。你呢，你在做什么？';
  // Contact
  if(/怎么联系|联系方式|邮箱|email|contact|怎么.*联系/i.test(text))
    return '他的邮箱是 hank.zihao@gmail.com 📧';
  // Real name
  if(/真名|本名|叫什么名字|实名|原名|中文名|英文名|real name|actual name/i.test(text))
    return '他叫张子豪（Zihao Zhang），英文名 Hank。';
  // Drinks
...
/* Admin tabs */
.admin-tab{{background:none;border:none;color:#868e96;padding:10px 20px;font-size:13px;cursor:pointer;border-bottom:2px solid transparent;transition:all .2s;font-weight:500}}
.admin-tab:hover{{color:#495057}}
.admin-tab.active{{color:#16181a;border-bottom-color:#4dabf7}}
/* Profile cards */
...
window.loadInsights=async function(){{
  var panel=document.getElementById('admin-insights');
  try{{
    ...
    panel.innerHTML='<div style="font-size:12px;color:#dc3545;padding:8px">Insights temporarily unavailable.</div>';
    panel.style.display='block';
  }}
}};
